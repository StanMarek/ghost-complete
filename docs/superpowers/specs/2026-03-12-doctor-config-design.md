...
2. **`gc-config/Cargo.toml`**: No change needed — `serde = { workspace = true }` with `derive` feature is already a direct dependency.
...
Requires re-exporting `parse_style` from `gc-pty` (which already depends on `gc-overlay`). Add `pub use gc_overlay::parse_style;` to `gc-pty/src/lib.rs`. This is a trade-off: re-exporting a rendering primitive through the PTY crate is semantically imperfect, but it avoids adding `gc-overlay` as a direct dependency of `ghost-complete` just for one validation function. Since `gc-pty` already orchestrates overlay rendering at runtime, the coupling is not new — only the visibility is.

#### Check 4: Shell integration installed

- Read `~/.zshrc` — always `~/.zshrc`, not `$ZDOTDIR/.zshrc`, because `ghost-complete install` writes to `~/.zshrc` unconditionally. The doctor check must mirror what install does.
...
enum Severity {
    ...
    Warn,
    Fail,
    Skip,  // grey/dim — check could not run due to prior failure
}
...
`Skip` does not affect the exit code. It is displayed as `[SKIP]` in dim/grey text.
...
**Exit code mechanism:** `run_doctor` calls `std::process::exit(1)` internally if any FAILs are present, matching how `run_validate_specs` already uses `std::process::exit(1)` on failures. On success it returns `Ok(())`.
...
No changes. `serde = { workspace = true }` with `derive` feature is already present.
