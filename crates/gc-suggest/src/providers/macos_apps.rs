...
async fn run_mdfind(cwd: &Path, binary: &str) -> Option<String> {
    ...
    .await
    {
        ...
        Err(e) => {
            tracing::trace!(binary, error = %e, "mdfind for applications failed");
            None
        }
    }
}
...
async fn run_mdls_bundle_id(cwd: &Path, binary: &str, app_path: &str) -> Option<String> {
    ...
    .await
    {
        Ok(stdout) => Some(stdout),
        Err(e) => {
            tracing::trace!(app_path, error = %e, "mdls bundle id lookup failed");
            None
        }
    }
}
...
impl MacosBundleIdentifiers {
    pub(crate) async fn generate_with_binaries(
        ...
        for path in stdout
            ...
            .map(str::trim)
            .filter(|l| l.ends_with(".app"))
            .take(BUNDLE_ID_RESOLVE_CAP)
        {
            let Some(raw) = run_mdls_bundle_id(&ctx.cwd, mdls_binary, path).await else {
                continue;
            };
