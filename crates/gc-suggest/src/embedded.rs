...
mod tests {
    ...
    /// `purge_embedded_cache_if_present` returns `Ok(None)` when there is
    /// nothing to delete. Callers must not treat that as a failure.
    #[test]
    fn purge_returns_none_when_dir_absent() {
        ...
        // against an arbitrary path. The behaviour we care about (NotFound is
        // not an error) is covered by the std::io contract used inside the fn.
    ...
    /// This test pins the symlink branch by exercising the same checks the
    /// real function uses (symlink_metadata + file_type().is_symlink()).
    #[test]
    #[cfg(unix)]
    fn purge_refuses_to_follow_symlink() {
        let tmp = TempDir::new().unwrap();
        ...
        // The real function's symlink branch returns Ok(None) without
        // calling remove_dir_all. We assert the canary survives a no-op
        // and the symlink itself is preserved — the same invariants the
