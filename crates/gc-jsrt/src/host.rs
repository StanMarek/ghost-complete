...
pub(crate) fn install_fig_helpers<'js>(ctx: &Ctx<'js>) -> rquickjs::Result<()> {
    match ctx.eval::<(), _>(FIG_HELPERS_JS).catch(ctx) {
        ...
        Err(e) => {
            ...
            // Re-throw the exception onto the ctx so the worker's
            // `install_host_api(...).catch(&ctx)` (see worker.rs::run_job)
            // observes the pending JS exception and surfaces its message
            // and stack via `CaughtError`'s Display impl in the
            // user-facing diagnostic. Without re-throwing here, dropping
            // `e` would clear the slot and the worker would only see
            // `rquickjs::Error::Exception` (the opaque
            // `"Exception generated by QuickJS"` placeholder).
            Err(e.throw(ctx))
        }
    }
}
